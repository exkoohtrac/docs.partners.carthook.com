...
| **priority** <br><br><sub><sup>OPTIONAL</sup></sub> | `"priority"`: `0 → [0…n]`<br><br>Default value: **depends on the priority of assets added before.** <br><br>The range is from [0…n]. Lower number means higher priority. E.g. priority 0 means that it gets loaded before priority 1 etc. The prority must be unique in respect to other assets that are loading together. |
...
| **topic** <br><br><sub><sup>REQUIRED</sup></sub> | `"topic"`: `"checkouts/created"` <br><br> Event that triggers the Webhook. <br><br> Valid values are: <br><br> • `checkouts/created`,`checkouts/updated`,`checkouts/completed` <br><br> • `apps/uninstalled` |
